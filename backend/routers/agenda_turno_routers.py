...
def crear_agenda():
    data = request.json
    agenda = service.crear_agenda(data)
    if agenda:
...
def obtener_por_id(agenda_id):
    agenda = service.obtener_por_id(agenda_id)
    if agenda:
...
def modificar(agenda_id):
    data = request.json
    agenda = service.modificar(agenda_id, data)
    if agenda:
...
def eliminar(agenda_id):
    success = service.eliminar(agenda_id)
    if success:
