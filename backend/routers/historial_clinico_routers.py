...
def listar_historiales():
    historiales = service.obtener_todos()
...
def obtener_historial(historial_id):
    historial = service.obtener_por_id(historial_id)
    if not historial:
...
def crear_historial():
    data = request.json
    nuevo_historial = service.crear_historial(data)
...
def actualizar_historial(historial_id):
    data = request.json
    historial_actualizado = service.actualizar_historial(historial_id, data)
...
def eliminar_historial(historial_id):
    eliminado = service.eliminar_historial(historial_id)
    if not eliminado:
